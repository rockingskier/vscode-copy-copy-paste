...
const getDocument = (editor: vscode.TextEditor): vscode.TextDocument =>
  editor.document;

const getEol = (document: vscode.TextDocument): string =>
  document.eol === 1 ? "\n" : "\r\n";
...
const buildHistoryCommand = (history: History) => () => {
  ...
  if (!items.length) {
    vscode.window.showInformationMessage(
      "There is currently nothing stored in your clipboard history."
    );
    return;
  }
  ...
  vscode.window
    ...
    .then((item?: ClipBoardItem) => {
      ...
      const mapBlocksToSelections: boolean =
        editor.selections.length === item.blocks.length;
      const mapLinesToSelections: boolean =
        item.blocks.length === 1 &&
        editor.selections.length === item.lines.length;

      const edits: Array<vscode.TextEdit> = editor.selections.map(
        (selection, index) => {
...
export function activate(context: vscode.ExtensionContext) {
  ...
  const copyCommand: vscode.Disposable = vscode.commands.registerCommand(
    "copy-copy-paste.copy",
    buildCopyCutCommand(history, false)
  );
  const cutCommand: vscode.Disposable = vscode.commands.registerCommand(
    "copy-copy-paste.cut",
    buildCopyCutCommand(history, true)
  );
  const pasteCommand: vscode.Disposable = vscode.commands.registerCommand(
    "copy-copy-paste.history",
    buildHistoryCommand(history)
  );
  const clearCommand: vscode.Disposable = vscode.commands.registerCommand(
    "copy-copy-paste.clear",
    () => history.clear()
  );
